...
try:
  if success: # full split ingestions successfully appendend
    ...
    if data_stream != None:
      ...
      portal.DataLake_invalidateReference(data_stream)
      data_stream.invalidate()
      if not portal.DataLake_checkReferenceInvalidated(data_ingestion):
        portal.DataLake_invalidateReference(data_ingestion)
      data_an = portal_catalog.getResultValue(
        portal_type = 'Data Analysis',
        id = data_stream.getId())
      if data_an != None:
        portal.DataLake_invalidateReference(data_an)
      data_array = portal_catalog.getResultValue(
        portal_type = 'Data Array',
        id = data_stream.getId())
      if data_array != None:
        portal.DataLake_invalidateReference(data_array)
    ...
    for data_stream in portal_catalog(portal_type = "Data Stream",
                                         ...
                                         reference = reference):
      if not portal.DataLake_checkReferenceInvalidated(data_stream):
        portal.DataLake_invalidateReference(data_stream)
