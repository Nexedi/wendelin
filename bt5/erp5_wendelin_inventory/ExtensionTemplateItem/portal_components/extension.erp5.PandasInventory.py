...
class ZBigArrayConverter(object):
  ...
  def convert(self, title=None, overwrite=False, add_category=True):
    '''
      ...
      Its parameters are:
        ...
        - title: the title of the Data Array which will be created from the 
          input;
    ...
    title = title or 'Wendelin + Jupyter'
    result = self.context.portal_catalog(title=title, portal_type='Data Array')
    if len(result) == 0:
...
def convertResultsToDataArray(self, results, title='Wendelin + Jupyter'):
  converter = ZBigArrayConverter(results, self)
  array = converter.convert(title=title)
  return array
...
class CategoryProcessor(object):
  ...
  def __init__(self, array_title, context):
  ...
  def fillCategoryList(self, fields=None, verbose=False, duplicate_category=False):
    ...
    result = self.context.portal_catalog(portal_type='Data Array', title=self.array_title)
...
def fillCategoryList(self, title, fields=None, verbose=False, duplicate_category=False):
  return CategoryProcessor(title, self).fillCategoryList(
    verbose=verbose, 
...
def getInventoryDataFrame(self, data_array_title=None, duplicated_categories=False, as_csv=False,**kw):
  if not data_array_title:
    if duplicated_categories:
      data_array_title = 'Wendelin + Jupyter (duplicated)'
    else:
      data_array_title = 'Wendelin + Jupyter'
  array = self.getPortalObject().portal_catalog(
      portal_type='Data Array', 
      title=data_array_title
